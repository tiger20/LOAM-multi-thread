...
void BasicScanRegistration::processScanlines(const Time& scanTime, std::vector<pcl::PointCloud<pcl::PointXYZI>> const& laserCloudScans)
{
  ...
  for (int i = 0; i < laserCloudScans.size(); i++) 
  {
    ...
    _scanIndices.push_back(range);//每根线的index range
  }
...
void BasicScanRegistration::processScanlines(std::vector<pcl::PointCloud<pcl::PointXYZI>> const& laserCloudScans)
{
  // reset internal buffers and set IMU start state based on current scan time
  reset();  
  ...
  {
    _laserCloud += laserCloudScans[i];
    //cout<<laserCloudScans[8].points.size()<<endl;
    IndexRange range(cloudSize, 0);
    cloudSize += laserCloudScans[i].size();
    range.second = cloudSize > 0 ? cloudSize - 1 : 0;
    _scanIndices.push_back(range);//每根线的index range
...
void BasicScanRegistration::reset()
{
  //_scanTime = scanTime;
...
void BasicScanRegistration::extractFeatures(const uint16_t& beginIdx)
{
  ...
  size_t nScans = _scanIndices.size();//nScans = 16
  //cout<<"nScans: "<<nScans<<endl;
  pcl::PointCloud<pcl::PointXYZI> splfs;
  for (size_t i = beginIdx; i < nScans; i++) 
  {
    //cout<<"extractFeatures. "<<i<<endl;
    ...
    // reset scan buffers
    setScanBuffersFor(scanStartIdx, scanEndIdx);//给_scanNeighborPicked赋值

    // extract features from equally sized scan regions
    for (int j = 0; j < _config.nFeatureRegions; j++) //对于第i条线提取特征
    {
      //_config.curvatureRegion = 5,nFeatureRegions = 6
      ...
      setRegionBuffersFor(sp, ep);//计算sp~ep之间的点的曲率并排序


      // extract corner features
      int largestPickedNum = 0;
      for (size_t k = regionSize; k > 0 && largestPickedNum < _config.maxCornerLessSharp;) //maxCornerLessSharp = 10*maxCornerSharp = 20
      {
...
void BasicScanRegistration::setScanBuffersFor(const size_t& startIdx, const size_t& endIdx)
{
  ...
  _scanNeighborPicked.assign(scanSize, 0);//长度为scanSize的int数组，初值为0
  ...
  for (size_t i = startIdx + _config.curvatureRegion; i < endIdx - _config.curvatureRegion; i++) 
  {
    ...
    float diffNext = calcSquaredDiff(nextPoint, point);//计算delta_x^2+delta_y^2+delta_z^2
